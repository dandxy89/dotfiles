...
local plugins = {
    ...
    { url = "https://github.com/nvim-tree/nvim-web-devicons",                 name = "nvim-web-devicons",           opt = false },
    ...
    -- (lazy)
    { url = "https://github.com/nvim-treesitter/nvim-treesitter",             name = "nvim-treesitter",             opt = true, trigger = "file" },
    { url = "https://github.com/nvim-treesitter/nvim-treesitter-textobjects", name = "nvim-treesitter-textobjects", opt = true, trigger = "file" },
    { url = "https://github.com/neovim/nvim-lspconfig",                       name = "nvim-lspconfig",              opt = true, trigger = "file" },
    { url = "https://github.com/williamboman/mason.nvim",                     name = "mason.nvim",                  opt = true, trigger = "file" },
    { url = "https://github.com/williamboman/mason-lspconfig.nvim",           name = "mason-lspconfig.nvim",        opt = true, trigger = "file" },
    { url = "https://github.com/chrisgrieser/nvim-lsp-endhints",              name = "nvim-lsp-endhints",           opt = true, trigger = "file" },
    ...
    { url = "https://github.com/lewis6991/gitsigns.nvim",                     name = "gitsigns.nvim",               opt = true, trigger = "git" },
    ...
    { url = "https://github.com/Saecki/crates.nvim",                          name = "crates.nvim",                 opt = true, trigger = "filetype:toml" },
}
...
local function setup_lazy_loading()
    ...
    vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" }, {
        once = true,
        callback = function()
            if loaded_groups.git then return end

            vim.defer_fn(function()
                if vim.fn.finddir(".git", vim.fn.expand("%:p:h") .. ";") ~= "" then
                    loaded_groups.git = true

                    for _, plugin in ipairs(plugins) do
                        if plugin.opt and plugin.trigger == "git" then
                            vim.cmd("packadd " .. plugin.name)
                        end
                    end
    ...
    vim.api.nvim_create_autocmd("FileType", {
        pattern = "toml",
        callback = function()
            if vim.fn.expand("%:t"):match("Cargo%.toml") then
                vim.cmd("packadd crates.nvim")
                require("plugins.config.crate")
            end
        end
    })
end
