...
function M.ensure_plugins_startup(plugins)
  ...
  if #missing > 0 then
    vim.notify('Found ' .. #missing .. ' missing plugins. They will be installed automatically by vim.pack.', vim.log.levels.INFO, { title = 'Plugin Manager' })
  end
end
...
function M.setup_command(plugins)
  vim.api.nvim_create_user_command('PackInstall', function()
    ...
    desc = 'Install missing plugins using vim.pack',
  })
end
