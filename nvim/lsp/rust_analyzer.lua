return {
    cmd = {
        'sh', '-c',
        'if rust-analyzer --version >/dev/null 2>&1; then exec rust-analyzer "$@"; ' ..
            'else exec rustup run nightly rust-analyzer "$@"; fi',
        'rust-analyzer'
    },
    filetypes = {'rust'},
    root_dir = function(bufnr, on_dir)
        ...
        vim.system({
            ...
            if output.code == 0 and output.stdout then
                ...
                on_dir(ok and result.workspace_root and
                           vim.fs.normalize(result.workspace_root) or
                           cargo_crate_dir)
            else
